...
describe('Controller: ResultsCtrl', function () {
  ...
  describe('tests with 2 artists results', function () {
    ...
    it('should have 2 records', function () {
      resultsController();
      $httpBackend.flush();
  ...
  describe('tests with 1 artist result', function () {
    beforeEach(inject(function($injector) {

      // Set up the mock http service responses
      $httpBackend = $injector.get('$httpBackend');
      // backend definition common for all tests
      $httpBackend.whenGET('http://ws.audioscrobbler.com/2.0/?api_key=aceab0ee7a1b569b1cec10bac112a84e&artist=korn&format=json&limit=24&method=artist.search&page=1')
        .respond({
          'results': {
    ...
    it('should show 1 match found', function () {
      resultsController();
      $httpBackend.flush();
      expect($rootScope.total).toBe(1);
      expect($rootScope.rows.length).toBe(1);
    });
